...
module.exports = {
  ...
  updateProjectiles: function(projectiles){
    for (var i in projectiles){
      ...
      } else if (projectiles[i].progress == 1000){
        projectiles.splice(i, 1);
      } else {
        projectiles[i].progress = projectiles[i].progress + 1;
        projectiles[i].X_pos = projectiles[i].X_origin + projectiles[i].x_distance * projectiles[i].progress/1000;
        projectiles[i].Y_pos = projectiles[i].Y_origin + projectiles[i].y_distance * projectiles[i].progress/1000;
